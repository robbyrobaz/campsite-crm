# Token Usage
**Updated:** 2026-03-18 04:30 MST

## ⚡ Anthropic Rate Limits (Live)
| Window | Usage | Resets At |
|--------|-------|-----------|
| 5-hour | **1.0%** | Mar 18 09:00 AM MST |
| 7-day (all) | **88.0%** 🔴 | Mar 19 08:59 PM MST |
| 7-day (Sonnet) | **90.0%** | Mar 19 08:59 PM MST |

## Rolling 5-Hour Window
- **Total Tokens:** 1.1M (19 requests)
- **Input:** 113 | **Output:** 4.4K
- **Cache Read:** 809.8K | **Cache Write:** 317.7K
- **Cost (est):** $1.50

### By Model (5h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 19 | 1.1M | 4.4K | $1.50 |

## Rolling 24-Hour Window
- **Total Tokens:** 67.7M (568 requests)
- **Output Tokens:** 133.4K
- **Cost (est):** $102.48

### By Model (24h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 379 | 52.3M | 93.2K | $75.09 |
| opus | 171 | 15.2M | 39.7K | $27.08 |
| haiku | 2 | 261.4K | 524 | $0.31 |
| delivery-mirror | 3 | 0 | 0 | $0.00 |
| llama-3.3-nemotron-super-49b-v1.5 | 9 | 0 | 0 | $0.00 |
| nemotron-mini-4b-instruct | 2 | 0 | 0 | $0.00 |
| nemotron-3-super-120b-a12b | 2 | 0 | 0 | $0.00 |

---
*Note: Max 5x plan is $100/mo flat. Cost shown is equivalent API pricing, not actual charges.*
*Live utilization from Anthropic OAuth usage API.*
