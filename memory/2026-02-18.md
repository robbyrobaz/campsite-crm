...
## HEADLESS BROWSER SETUP ✅ (10:30 MST)
...
**Verified working:** Navigated to example.com, clicked links, navigated to IANA — all snapshot/click/navigate working.

## JARVIS GOOGLE ACCOUNT ✅ (10:42 MST)

**Account:** jarvis.is.my.coo@gmail.com / is$$sseljs21
**Status:** Logged in via headless browser, session active
**Google Cloud Console:** Accessed, no project created yet — waiting for Rob's direction on what to set up
**Note:** No 2FA on this account — headless login works cleanly

## ARB DASHBOARD — LIVE ✅ (11:40 MST)
...
- Select sportsbooks → enter promos (type + amount only) → auto-finds best hedges across all games
- Top 10 results ranked by guaranteed profit with step-by-step instructions
- Tested: $500 DK free bet found $326 guaranteed profit on NBA game (DK→FD hedge)
- Production build served via `npx serve dist` (not vite dev server)

**Architecture:**
- `src/services/oddsApi.ts` — fetches NBA/NHL/MLB/NFL odds, caches 5 min
- `src/engine/hedgeCalculator.ts` — scans all games × all books, finds optimal hedges
- `src/store/useArbStore.ts` — Zustand with odds caching + fetch state
...
1. **Profit Boost %** (20%, 30%, 75%) — boost winnings by X%, min odds constraint, max bet ($5-$25)
2. **Odds Surge** (+400 flat added to odds) — different from % boost
3. **Scoring Stack** — escalating boost per parlay leg
4. **Second Chance** — stake back if specific condition (e.g., golfer finishes 2nd)
5. **Bet Reset** — theScore $1,000 first bet, 100% back as 5 bonus bets if lose (SNR)
6. **Deposit Bonus** — play-through tracker (DK up to $1,000)
7. **Profit Boost Packs** — multiple boosts bundled (FanDuel NBA Tip Off Pack = 3 boosts)

**Key insight from Rob:** Real workflow should be screenshot → auto-parse → instant hedge recommendations, NOT manual form entry. Most promos are $5-$25 with specific constraints (sport, game, bet type, min odds). Value comes from stacking many small promos across multiple books daily.

## GCP CLI + BROWSER SETUP ✅ (10:52 MST)
...
**Daily check:** Added to HEARTBEAT.md — verify job ran within 24h + exit 0
...
## ARB DASHBOARD — PROMO EXPANSION COMPLETE ✅ (12:39 MST)

**HedgeEngine PRO dashboard** at `/home/rob/.openclaw/workspace/arb-dashboard/` (localhost:5173)

**Promo types expanded from 4 → 8:**
1. Free Bet (SNR) — $ amount
2. Free Bet (Stake Returned) — $ amount
3. Deposit Match — $ amount
4. Risk-Free / First Bet Reset — $ amount
5. Profit Boost — % boost + $ max wager (dual fields)
6. Odds Boost — $ amount
7. Bet & Get — $ qualifying bet + $ bonus amount (dual fields)
8. Bonus Cash / Site Credit — $ amount
...
## HEDGEENGINE PUSHED TO GITHUB ✅ (13:04 MST)

**Repo:** https://github.com/robbyrobaz/hedge-engine (public)
**Branch:** main (initial release v1.0)
**27 files, 6,652 lines**

Sport filter dropdown added per promo (All Sports, NBA, CBB, NFL, CFB, MLB, NHL, MLS, EPL, UCL, Olympic Hockey). Sports list expanded from 4 → 10 to cover what's actually live.

**Hosting decision:** Vercel recommended but signup blocked by bot detection on headless browser. Fell back to GitHub Pages — zero friction.

## HEDGEENGINE DEPLOYED LIVE ✅ (13:16 MST)
...
**Rob's feedback on profit boost math:** $2,500 "max reward" on a $25 max wager 30% boost is just legal fine print. Real profit from hedging a 30% boost at $25 is ~$5-10 depending on odds. Rob caught this immediately — he knows the math.

## HEDGEENGINE ITERATIONS (13:23-14:02 MST)

**Merged Odds Boost into Profit Boost** — same math, now labeled "Profit / Odds Boost". Down from 8 to 7 promo types.

**Sport groups instead of individual leagues** — "Golf" covers all 4 tournaments, "Soccer" covers 7 leagues, etc. 9 clean categories with plain English labels (no acronyms). Rob requested this — doesn't want to add 4 separate golf promos.
...
**🚨 CRITICAL BUG FOUND & FIXED: 3-Way Market Draw Risk**
...
**Dev server fix:** `base` in vite.config.ts now conditional — `/` for dev, `/hedge-engine/` for production GitHub Pages build.
...
- Modified `/home/rob/.openclaw/workspace/blofin-dashboard/server.py` — added `top3_metrics` to `/api/advanced_metrics` endpoint
- Modified `/home/rob/.openclaw/workspace/blofin-dashboard/blofin-dashboard.html` — Advanced Trading Metrics section now shows Top 3 strategy averages instead of all-strategy totals
...
- paper_trades table has NO `strategy` column — only links via confirmed_signal_id → signals

## CPU Temperature Investigation (15:07 MST)
- Heartbeat flagged 86°C, blamed `claude` process at 210% CPU
- Rob correctly pointed out Claude inference is cloud-side — local CLI shouldn't be heavy
- Found 2 CLI instances: PID 3125 (running since Feb 16!) and PID 247822 (from 09:39)
- Rob closed both, temp dropped to 66°C immediately
...
- `gnome-system-monitor` not installed — likely removed during package cleanup
- Fix: `sudo apt install gnome-system-monitor`
- htop is installed as CLI alternative

## Blofin Pipeline Redesign — COMPLETE (17:20 MST)
...
1. **ML Fix:** `returns` feature removed (was leakage source, importance=1.0), temporal train/test split, scaler fit on train only, quarantine warning for F1≥0.95, ML gate in paper_engine.py. Entry ROC-AUC 0.577 (honest), Exit 0.784.
...
- Top backtest: mtf_momentum_confirm (76.5), mtf_trend_align (75.2), ml_gbt_5m (72.8) — no forward-test yet
- Top forward-test: momentum (10.7), vwap_reversion (9.5) — sobering reality without leakage
- volume_mean_reversion: best BT EEP (36.6) despite only 54% WR — proves profit factor > win rate
...
- CPU 58°C, disk 30%, no failed services, no critical alerts
- DB: 35.1M ticks, 45.7K signals, 35.9K paper trades, 52 registered strategies

## Dashboard Rebuild & 8780 Deprecation (17:30-17:40 MST)

**Port 8780 fully deprecated:**
- `blofin-stack-api.service` stopped, disabled, unit file removed
- `api_server.py` → `.deprecated` (along with dashboard_health_check.py, smoke_test_phase1.py)
- TOOLS.md updated, port confirmed not listening

**New dashboard at 8888 — complete rewrite:**
- Pipeline funnel visualization: T0 (43) → T1 (0) → T2 (9) → T3 (0)
- Strategy table sortable by EEP (default), filterable by tier
- ML model cards with LEAKAGE SUSPECTED warnings (direction_predictor, momentum_classifier)
- Live data feed with 30s auto-refresh
- All endpoints self-contained in server.py (no 8780 proxy)
- Endpoints: /api/registry, /api/ml_models, /api/summary, /api/advanced_metrics

**Commits:** `4d753a7` (dashboard rebuild), pushed to both repos

## Pipeline First Run & Strategy Interface Fix (18:05-18:15 MST)

**Pipeline runner first execution:** Completed in 126s, then optimized to 3s with smoke test.
- Phase 1: 5 new strategies registered (57 total)
- Phase 2: 0 promotions — ALL Tier 0 strategies produced 0 trades in backtest
- Phase 4: 8 Tier 2 strategies demoted to Tier 1 (poor EEP)
- Only `momentum` survived at Tier 2

**ROOT CAUSE: Tick vs Candle mismatch**
- Live signal engine feeds raw ticks (multiple/sec) → strategies see spikes → signals fire
- Backtester aggregates to OHLCV candles → smoothed data → thresholds never trigger
- Example: VWAP deviation threshold 1.2%, but max deviation on 1m candles is 0.93%
- All 9 active strategies were tuned for tick-level volatility

**Strategy adapter fix:** Updated StrategyAdapter to bridge old `detect(symbol, price, volume, ts_ms, prices_tuples, volumes_tuples)` to new `detect(context_candles, symbol)`. Still 0 trades because underlying threshold issue.

**Solution:** Rewriting all 12 strategies for OHLCV candle interface with calibrated thresholds (VWAP 0.3%, RSI 30/70, BB 20/2). Builder delegated.

## Numerai Tournament — New Project (18:27 MST)

**Repo:** https://github.com/robbyrobaz/numerai-tournament
- ML tournament where Rob gets paid based on model performance
- Currently runs on another machine, submits 3 pre-trained models daily
- Training takes 2-5 hours per model
- Real scores come back weekly (Wednesdays)
- Rob wants to use omen-claw's GPU to test more variations
- Builder delegated to analyze repo and create improvement plan

## Rate Limiting — 82 Hits Today (18:27 MST)

**Cause:** Too many concurrent sessions (agent teams + builders + heartbeats + pipeline cron)
**Fixes applied:**
- Pipeline cron: hourly → every 2 hours, switched from Haiku to Mini
- Disabled old morning/evening strategy adjustment crons (redundant with pipeline)
- Fewer concurrent sessions = less rate limit pressure

## Cron Job Cleanup (18:28 MST)

**Active crons (3):**
- Heartbeat: every hour at :00 (Haiku)
- Pipeline: every 2h at :15 (Mini)
- Token audit: weekly Sunday 8am (Mini)

**Disabled (2):**
- Blofin Strategy Adjustment - Morning (replaced by pipeline)
- Blofin Strategy Adjustment - Evening (replaced by pipeline)

## Dashboard Health Check Service

- `dashboard-health-check.service` was failed (stale from 8780 deprecation)
- Masked it to prevent recurring failed state
