# Token Usage
**Updated:** 2026-04-06 04:45 MST

## ⚡ Anthropic Rate Limits (Live)
| Window | Usage | Resets At |
|--------|-------|-----------|
| 5-hour | **5.0%** | Apr 06 07:00 AM MST |
| 7-day (all) | **47.0%** | Apr 09 09:00 PM MST |
| 7-day (Sonnet) | **54.0%** | Apr 10 08:00 AM MST |

## Rolling 5-Hour Window
- **Total Tokens:** 0 (373 requests)
- **Input:** 0 | **Output:** 0
- **Cache Read:** 0 | **Cache Write:** 0
- **Cost (est):** $0.00

### By Model (5h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 373 | 0 | 0 | $0.00 |

## Rolling 24-Hour Window
- **Total Tokens:** 2.3M (972 requests)
- **Output Tokens:** 14.6K
- **Cost (est):** $3.07

### By Model (24h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 972 | 2.3M | 14.6K | $3.07 |

---
*Note: Max 5x plan is $100/mo flat. Cost shown is equivalent API pricing, not actual charges.*
*Live utilization from Anthropic OAuth usage API.*
