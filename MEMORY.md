...
- **Retired NQ Overnight Monitor, Moonshot Dashboard Iteration** — absorbed into agent heartbeats
- **Created NQ + Crypto autonomous heartbeat crons** (every 30min, staggered)
- **Jarvis reduced to server-level health only** (CPU, disk, git backup, token usage)

### Agent File Architecture
- All agent files (SOUL, AGENTS, IDENTITY, BOOTSTRAP, MEMORY) symlinked from repos
- Sub-agents edit files in their repo → symlinks pipe to agentDir → loads at session boot
- Jarvis files live directly in `~/.openclaw/agents/main/agent/` (no symlinks needed)
- **Workspace root has NO identity files** — prevents context bleed between agents
...
Workspace-level SOUL.md was corrupted with "You are the CRYPTO AGENT" — bled into every agent's
context because all agents share the workspace. Fix: moved Jarvis files to agentDir, removed
workspace-level identity files.

### ⛔ NEVER put identity files (SOUL.md, AGENTS.md, etc.) in the workspace root
They will load as Project Context for ALL agents and cause identity confusion.
