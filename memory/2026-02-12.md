...
3. Updated systemd services:
   - `kanban-dashboard.service` → points to new location
   - `kanban-worker.service` → points to new location
4. Added `upsert_heartbeat()` function to kanban db.py
5. Cleared Python cache to force reload

**Services:**
- `kanban-dashboard.service` ✅ running on port 8781
- `kanban-worker.service` ✅ running
- `kanban-watchdog.timer` ✅ waiting
