# Token Usage
**Updated:** 2026-04-01 00:30 MST

## ⚡ Anthropic Rate Limits (Live)
| Window | Usage | Resets At |
|--------|-------|-----------|
| 5-hour | **9.0%** | Apr 01 03:00 AM MST |
| 7-day (all) | **85.0%** 🔴 | Apr 02 09:00 PM MST |
| 7-day (Sonnet) | **72.0%** | Apr 03 08:00 AM MST |

## Rolling 5-Hour Window
- **Total Tokens:** 7.8M (56 requests)
- **Input:** 154 | **Output:** 9.3K
- **Cache Read:** 6.0M | **Cache Write:** 1.7M
- **Cost (est):** $8.46

### By Model (5h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 56 | 7.8M | 9.3K | $8.46 |

## Rolling 24-Hour Window
- **Total Tokens:** 21.7M (230 requests)
- **Output Tokens:** 52.5K
- **Cost (est):** $23.69

### By Model (24h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 212 | 21.1M | 38.1K | $23.69 |
| nvidia/nemotron-3-super-120b-a12b:free | 12 | 515.4K | 9.7K | $0.00 |
| deepseek/deepseek-r1-0528 | 1 | 21.5K | 4.7K | $0.00 |
| qwen/qwen3-coder:free | 4 | 0 | 0 | $0.00 |
| qwen/qwen3.5-122b-a10b:free | 1 | 0 | 0 | $0.00 |

---
*Note: Max 5x plan is $100/mo flat. Cost shown is equivalent API pricing, not actual charges.*
*Live utilization from Anthropic OAuth usage API.*
