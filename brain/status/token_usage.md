# Token Usage
**Updated:** 2026-03-18 19:30 MST

## ⚡ Anthropic Rate Limits (Live)
| Window | Usage | Resets At |
|--------|-------|-----------|
| 5-hour | **0.0%** | Mar 19 12:00 AM MST |
| 7-day (all) | **94.0%** 🔴 | Mar 19 08:59 PM MST |
| 7-day (Sonnet) | **97.0%** | Mar 19 08:59 PM MST |

## Rolling 5-Hour Window
- **Total Tokens:** 1.0M (46 requests)
- **Input:** 879.9K | **Output:** 22.0K
- **Cache Read:** 27.7K | **Cache Write:** 75.9K
- **Cost (est):** $0.27

### By Model (5h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| nvidia/nemotron-3-super-120b-a12b:free | 38 | 900.8K | 20.9K | $0.00 |
| sonnet | 3 | 81.5K | 886 | $0.25 |
| haiku | 1 | 23.2K | 203 | $0.02 |
| deepseek-coder-v2:16b | 1 | 0 | 0 | $0.00 |
| qwen2.5-coder:14b | 2 | 0 | 0 | $0.00 |
| openrouter/nemo-super | 1 | 0 | 0 | $0.00 |

## Rolling 24-Hour Window
- **Total Tokens:** 1.0M (46 requests)
- **Output Tokens:** 22.0K
- **Cost (est):** $0.27

### By Model (24h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| nvidia/nemotron-3-super-120b-a12b:free | 38 | 900.8K | 20.9K | $0.00 |
| sonnet | 3 | 81.5K | 886 | $0.25 |
| haiku | 1 | 23.2K | 203 | $0.02 |
| deepseek-coder-v2:16b | 1 | 0 | 0 | $0.00 |
| qwen2.5-coder:14b | 2 | 0 | 0 | $0.00 |
| openrouter/nemo-super | 1 | 0 | 0 | $0.00 |

---
*Note: Max 5x plan is $100/mo flat. Cost shown is equivalent API pricing, not actual charges.*
*Live utilization from Anthropic OAuth usage API.*
