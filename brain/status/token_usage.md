# Token Usage
**Updated:** 2026-03-23 23:30 MST

## ⚡ Anthropic Rate Limits (Live)
| Window | Usage | Resets At |
|--------|-------|-----------|
| 5-hour | **8.0%** | Mar 24 12:00 AM MST |
| 7-day (all) | **51.0%** | Mar 26 08:59 PM MST |
| 7-day (Sonnet) | **37.0%** | Mar 27 08:00 AM MST |

## Rolling 5-Hour Window
- **Total Tokens:** 2.3M (33 requests)
- **Input:** 224 | **Output:** 21.1K
- **Cache Read:** 1.5M | **Cache Write:** 745.8K
- **Cost (est):** $3.50

### By Model (5h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 30 | 2.2M | 20.7K | $3.47 |
| haiku | 2 | 40.9K | 451 | $0.03 |
| delivery-mirror | 1 | 0 | 0 | $0.00 |

## Rolling 24-Hour Window
- **Total Tokens:** 11.5M (131 requests)
- **Output Tokens:** 52.5K
- **Cost (est):** $17.94

### By Model (24h)
| Model | Requests | Tokens | Output | Cost |
|-------|----------|--------|--------|------|
| sonnet | 117 | 11.2M | 48.3K | $17.56 |
| opus | 8 | 194.1K | 3.4K | $0.33 |
| haiku | 4 | 81.7K | 832 | $0.06 |
| delivery-mirror | 2 | 0 | 0 | $0.00 |

---
*Note: Max 5x plan is $100/mo flat. Cost shown is equivalent API pricing, not actual charges.*
*Live utilization from Anthropic OAuth usage API.*
